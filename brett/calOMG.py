...
for e in events:
    ...
    elif json.loads(check) != info:
        index = info.index(e["preformatted_localStartDate"])

        if id in checkedID and info[index] != json.loads(check)[index]:
            info[index] = newDates(json.loads(check)[index], info[index], str(e["name"]))
            set = True
        elif id in checkedID or info[index] not in json.loads(check)[index]:
            set = True
        elif info[index] in json.loads(check)[index] and any(info[i] != json.loads(check)[i] for i in range(len(info)) if i != index):
            set = True

    if set:
        newInfo = formatinfo(info)
        mapping = {
            "name": newInfo[0],
            "date": newInfo[1],
            "time": newInfo[2],
        ...
        #db.hset("events", id, mapping=mapping)
        db.hset("events", None, None, mapping)
        #db.json().set(id, "$", mapping)
